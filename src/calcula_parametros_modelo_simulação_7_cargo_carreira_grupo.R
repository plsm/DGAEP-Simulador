...
calcula.parametros.modelo.simulação <- function (
  ...
  ciclo.administração <- function (
    ...
    return (sub.dados [
      ,
      ciclo.cargo.carreira.grupo (cargo, .SD),
      by = .(cargo)
    ])
  }
  ciclo.cargo.carreira.grupo <- function (
    nome.cargo.carreira.grupo,
    sub.dados
  ) {
    cat (sprintf ("  %s\n", nome.cargo.carreira.grupo))
    return (sub.dados [
  ...
  fwrite (
    ...
    file = pasta.dados (
      sprintf (
        "parametros-modelo-7_postos-trabalho-por-cargo-carreira-grupo_delta=%s.csv",
        str_remove (
          sprintf ("%.1f", delta),
          "[.]?0+$"
        )
